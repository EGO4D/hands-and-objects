...
class CanonicalKeyframeLocalisation(torch.utils.data.Dataset):
    """
    ...
    def _construct_loader(self):
        ...
        for video_id in tqdm(self.ann_data.keys(), desc='+ive clips'):
            ...
            for interval in intervals:
                ...
                for action in interval_narrated_actions:
                    if 'is_rejected' not in action.keys() and 'error' not in \
                        action.keys() and 'is_invalid_annotation' not in \
                            action.keys():
                        if action['critical_frame_data'] is not None:
                            narration_uid = action['narration_annotation_uid']
                            clip_start_sec_str = str(
                                action['parent_start_sec']
                            ).replace('.', '_')
                            clip_end_sec_str = str(
                                action['parent_end_sec']
                            ).replace('.', '_')
                            ...
                            # if new_uid == '':
                                ...
                                pnr_frame = action['critical_frame_data'][
                                    'pnr_frame'
                                ]
                                if clip_start_frame > pnr_frame:
                                    # Ignoring such cases; there are 18 cases as
                                    # of 25-08-21
                                    continue
                                state = action['critical_frame_state_transition']
                                if state is None:
                                    # Ignoring such cases; there are 4303 such
                                    # cases as of 25-08-21
                                    continue
                                f_verb = action['critical_frame_freeform_verb']
                                s_verb = action['critical_frame_structured_verb']
        ...
        if self.no_state_chng:
            ...
            for video_id in tqdm(no_sc_video_ids, desc='-ive clips'):
                clip_count += 1
                ...
                assert os.path.isdir(clip_path)
        ...
        print(f'Number of clips for training: {len(self.package)}')
    ...
    def __getitem__(self, index):
        ...
        if info['pnr_frame'] is not None:
            ...
            state = self.state_mapping[info["state"]]
        else:
    ...
    def _sample_frames(
        ...
        if num_frames < num_frames_required:
            print(f'Possible issue with {unique_id}')
    ...
    def _sample_frames_gen_labels(self, info):
        ...
        if self.mode == 'test':
            ...
            if not os.path.isfile(self.cfg.TEST.JSON):
                ...
                self.test_det_data[info['unique_id']] = {
                    'clip_end_frame': random_end_frame,
                    'clip_start_frame': random_start_frame
                }
        ...
        if pnr_frame is not None:
            assert random_start_frame <= pnr_frame <= random_end_frame
        else:
            assert random_start_frame < random_end_frame
        ...
        for frame_num in candidate_frame_nums:
            ...
            assert os.path.isfile(frame_path), f'{frame_path}; {candidate_frame_nums}'
